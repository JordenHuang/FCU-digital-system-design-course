      ...
      CASE state IS
        ...
        WHEN translate =>
            ...
            ELSE --if control keys are not pressed  
              ...
              case ps2_code is
              -- if (e0_code = '1') then
                  -- when x"75" => ascii <= x"7A";
                  -- when x"6B" => ascii <= x"7B";
                  -- when x"72" => ascii <= x"7C";
                  -- when x"74" => ascii <= x"7D";
                  when x"75" => ascii <= x"41"; -- A, UP
                  when x"6B" => ascii <= x"44"; -- D, LEFT
                  when x"72" => ascii <= x"42"; -- B, DOWN
                  when x"74" => ascii <= x"43"; -- C, RIGHT
                  WHEN OTHERS => NULL;
                -- end if;
                end case;
